describe("TC_04.1 - Measure Login Page Load and Login Submission Time", () => {
    ...
    cy.visit("https://opensource-demo.orangehrmlive.com/").then(() => {
      ...
      cy.url({ timeout: 10000 })
        ...
        .then(() => {
          ...
          // Optional logging
          if (
            pageLoadTime > thresholds.pageLoad ||
            loginTime > thresholds.loginTime
          ) {
            cy.writeFile(
              "cypress/logs/performance_log.txt",
              ` Page Load: ${pageLoadTime}ms | Login Time: ${loginTime}ms\n`,
              { flag: "a+" }
            );
